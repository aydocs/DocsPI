...
mod registry {
    ...
    pub fn set_proxy(proxy_addr: &str, port: u16, extra_bypass: &[String]) -> Result<(), String> {
        ...
        let mut base_bypass: Vec<&str> = vec![
            ...
            "*.discord.media",
            "gateway.discord.gg",
            "discord-attachments-*.s3.amazonaws.com",
            // EA / Origin
            "*.ea.com",
            "*.origin.com",
            // Blizzard / Battle.net
...
fn set_system_proxy(port: u16, enable_winhttp: bool, custom_bypass_domains: Vec<String>) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if !registry::can_access() {
            return Err(
                "Registry erişim izni yok. Lütfen sistem izinlerini kontrol edin.".to_string(),
            );
        }
...
/// Task Scheduler'da DocsPI görevi var mı kontrol eder
#[tauri::command]
...
fn get_isp_info() -> IspInfo {
    ...
    #[cfg(target_os = "windows")]
    {
            ...
            .output()
        {
            ...
            if adapter_name.contains("wi-fi") || adapter_name.contains("wireless") || adapter_name.contains("wlan") || adapter_name.contains("wi_fi") {
                connection_type = "wifi".to_string();
            } else if adapter_name.contains("ethernet") || adapter_name.contains("lan") {
                connection_type = "ethernet".to_string();
            } else if adapter_name.contains("wwan") || adapter_name.contains("mobile") || adapter_name.contains("lte") || adapter_name.contains("cellular") {
            ...
            if let Ok(geo) = serde_json::from_str::<serde_json::Value>(&geo_txt) {
                if let Some(isp_str) = geo.get("isp").and_then(|v| v.as_str()) {
                    ...
                    if !isp_lower.contains("not available") && !isp_str.is_empty() {
                        ...
                        if name.contains("turktelekom") || name.contains("turktelekomunikasyon") {
                            name = "turktelekom".to_string();
                        } else if name.contains("superonline") {
                            name = "superonline".to_string();
                        } else if name.contains("vodafone") {
                            name = "vodafone".to_string();
                        } else if name.contains("turknet") {
                            name = "turknet".to_string();
                        } else if name.contains("kablonet") {
                            name = "kablonet".to_string();
                        } else if name.contains("milenicom") {
                            name = "milenicom".to_string();
                        }
                    }
                }
                ...
                } else if let Some(region_name) = geo.get("regionName").and_then(|v| v.as_str()) {
...
fn auto_connect_config() -> AutoConnectConfig {
    ...
    let (dns, fallback_dns_chain) = match isp.name.as_str() {
        "turktelekom" => ("cloudflare", vec!["cloudflare", "quad9", "google"]),
        "superonline" => ("google", vec!["google", "cloudflare", "quad9"]),
        "vodafone" => ("cloudflare", vec!["cloudflare", "quad9", "google"]),
        "turknet" => ("cloudflare", vec!["cloudflare", "quad9", "google"]),
        "kablonet" => ("cloudflare", vec!["cloudflare", "google", "quad9"]),
        "milenicom" => ("cloudflare", vec!["cloudflare", "quad9", "google"]),
        _ => ("cloudflare", vec!["cloudflare", "quad9", "google"]),
    };
...
fn get_driver_info() -> serde_json::Value {
    ...
    // WinDivert.dll versiyonunu kontrol etmeye çalış
    let version = if installed {
        #[cfg(target_os = "windows")]
        {
            // WinDivert versiyonunu dll üzerinden çekmek karmaşık olabilir, 
            // şimdilik en güncel desteklenen versiyonu dönüyoruz.
            "2.2.2"
        }
        #[cfg(not(target_os = "windows"))]
        "N/A"
...
pub fn run() {
    #[cfg(target_os = "windows")]
    {
        ...
        let mutex_name: Vec<u16> = "Global\\DocsPI_SingleInstance\0".encode_utf16().collect();

        unsafe {
            let handle = CreateMutexW(null_mut(), 0, mutex_name.as_ptr());
            if handle.is_null() || GetLastError() == ERROR_ALREADY_EXISTS {
                eprintln!("[STARTUP] DocsPI zaten çalışıyor — çıkılıyor");
