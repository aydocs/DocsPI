...
func processTCPOutbound(h syscall.Handle, pkt []byte, addr []byte, cfg *Config) {
	...
	if cfg.WrongChksum {
		sendFakeWrongChksum(h, pkt, addr, cfg)
	}

	if cfg.WrongSeq {
		sendFakeWrongSeq(h, pkt, addr, cfg)
	}
...
func sendFakeWrongChksum(h syscall.Handle, pkt []byte, addr []byte, cfg *Config) {
...
func sendFakeWrongSeq(h syscall.Handle, pkt []byte, addr []byte, cfg *Config) {
	ihl := ipv4HeaderLen(pkt)
	thl := tcpDataOffset(pkt)
	headerSize := ihl + thl
